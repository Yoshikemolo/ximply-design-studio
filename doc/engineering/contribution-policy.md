...
quality gate. GitHub's default merge button creates a web-flow committer, even when
the person clicking it is the owner. Editing the merge title alone does not correct
that identity. Integration must preserve validated commits under the existing
GitFlow/review rules; never force-push or bypass required protections.
