...
export type LayoutBlock = "tools" | "appearance" | "workspace" | "measurement" | "dimensions" | "pivot" | "selection" | "swatches" | "contextBar" | "documentTabs";
...
export class PreferencesService {
  ...
  readonly layoutBlocks = signal<Record<LayoutBlock, boolean>>({ tools: true, appearance: true, workspace: true, measurement: true, dimensions: true, pivot: true, selection: true, swatches: false, contextBar: true, documentTabs: true });
  ...
  constructor() {
    try {
      ...
      if (raw) {
        ...
        for (const key of ["appearance", "workspace", "measurement", "dimensions", "swatches", "contextBar", "documentTabs"] as const)
