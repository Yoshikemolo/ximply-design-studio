...
it('isolates modal Escape from editor actions and exposes smart dimension as default subtool', () => {
 ...
 const family=TOOL_FAMILIES.find(f=>f.id==='dimensions')!;expect(family.tools).toEqual(['dimensionSmart','dimensionLinear','dimensionAngular','dimensionChain']);expect(TOOLS.filter(t=>family.tools.includes(t.id))).toHaveLength(4);
});
