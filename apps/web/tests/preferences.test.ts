...
describe("input preferences", () => {
  ...
  it("persists measurement units, visibility and individual layout blocks", () => {
    ...
    expect(restored.layoutBlocks()).toEqual({ tools: true, appearance: false, workspace: true, measurement: true, dimensions: true, pivot: true, selection: true, swatches: false });
  });
