...
class Layer(Point):
    ...
    def non_nullable_extensions(cls, value):
        if isinstance(value, dict) and any(key in value and value[key] is None for key in ('curves', 'symbolId', 'traceSourceId', 'groupPath', 'flipX', 'flipY', 'skewX', 'guide', 'textLayout', 'typography', 'strokeStyle', 'lineEnds', 'brushStroke', 'fillPaint', 'envelope', 'gradientMesh', 'dimension', 'regroupPath', 'procedural')):
