...
describe('procedural parameter contracts and regeneration',()=>{
 ...
 it('uses cubic circular swing arcs with the correct radius and preserves a free portal when angle changes',()=>{const initial=generateProcedural(make('door'));const arc=initial.curves!.find(p=>p.nodes.length===2&&(p.nodes[0].outgoing.x!==p.nodes[0].point.x||p.nodes[0].outgoing.y!==p.nodes[0].point.y))!;expect(arc).toBeDefined();const jambs=initial.curves!.slice(0,2).flatMap(p=>p.nodes.map(n=>worldPoint(initial,n.point)));const center={x:jambs.reduce((sum,p)=>sum+p.x,0)/jambs.length,y:jambs.reduce((sum,p)=>sum+p.y,0)/jambs.length};if(initial.procedural?.type!=='door')throw Error('door');const updated=generateProcedural({...initial,procedural:{...initial.procedural,openingAngle:45}});const points=updated.curves!.slice(0,2).flatMap(p=>p.nodes.map(n=>worldPoint(updated,n.point)));expect(points.reduce((s,p)=>s+p.x,0)/points.length).toBeCloseTo(center.x);expect(points.reduce((s,p)=>s+p.y,0)/points.length).toBeCloseTo(center.y);const n=arc.nodes;const midpoint=cubic(n[0].point,n[0].outgoing,n[1].incoming,n[1].point,.5);const hinge=initial.curves![2].nodes[0].point;expect(Math.hypot(midpoint.x-hinge.x,midpoint.y-hinge.y)).toBeCloseTo(80,1);});
