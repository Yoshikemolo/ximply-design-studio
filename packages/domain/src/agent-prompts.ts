...
export interface SavedPrompt { id: string; title: string; prompt: string; action: AgentAction; creativity: number; favorite: boolean }
...
/** The prompts every user starts with; they can be marked favourite but not changed or removed. */
export const DEFAULT_PROMPTS: readonly SavedPrompt[] = [
  ...
  { id: "default-logo", title: "Logo to paths", prompt: "Keep the shapes of the logo and its colours exactly.", action: "vectorize", creativity: 0.2, favorite: false },
];
...
export function promptLibrary(saved: readonly SavedPrompt[]): LibraryPrompt[] {
  ...
  const defaults = DEFAULT_PROMPTS.map((prompt) => ({ ...prompt, favorite: favourites.has(prompt.id), builtIn: true }));
...
/** Removes a saved prompt; defaults cannot be removed. */
export function removePrompt(saved: readonly SavedPrompt[], id: string): SavedPrompt[] {
  return isBuiltIn(id) ? [...saved] : saved.filter((prompt) => prompt.id !== id);
}
