...
describe('dimension Canvas and SVG geometry',()=>{
 ...
 it('renders independent extension and dimension colors at matching world coordinates',async()=>{for(const ctx of await images(fixture())){expect([...ctx.getImageData(20,30,1,1).data]).toEqual([0,0,255,255]);expect([...ctx.getImageData(60,50,1,1).data]).toEqual([255,0,0,255]);expect([...ctx.getImageData(60,30,1,1).data]).toEqual([255,255,255,255]);}});
